...
/// No-op debug macro (replaces yaml-rust2's debug module).
...
impl<T: Iterator<Item = char>> Scanner<T> {
    ...
    fn scan_flow_scalar(&mut self, single: bool) -> Result<Token, ScanError> {
        ...
        loop {
            ...
            // CloudFormation and PyYAML accept quoted scalars with
            // continuation lines at any indentation level.
            // yaml-rust2 originally rejected this; we allow it.
    ...
    fn fetch_value(&mut self) -> ScanResult {
        ...
        if sk.possible {
            ...
            if let Some(sk_entry) = self.simple_keys.last_mut() {
                sk_entry.possible = false;
            }
            self.disallow_simple_key();
        } else {
            if self.implicit_flow_mapping {
    ...
    fn unroll_indent(&mut self, col: isize) {
        ...
        while self.indent > col {
            let indent = match self.indents.pop() {
                Some(i) => i,
                None => break,
            };
            self.indent = indent.indent;
