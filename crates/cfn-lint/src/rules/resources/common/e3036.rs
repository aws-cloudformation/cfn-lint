...
impl CfnLintRule for E3036 {
    ...
    fn validate(
        ...
        // Get the resource name from path: Resources/<name>/UpdateReplacePolicy
        ...
        let mut valid = vec!["Delete", "Retain"];
        if SNAPSHOT_TYPES.contains(&resource_type) {
            valid.push("Snapshot");
        }
        let schema = serde_json::json!({"type": "string", "enum": valid});

        // Only allow functions that CFN supports in UpdateReplacePolicy
...
crate::register_cfn_lint_rule!(E3036);
