...
static POLICIES: LazyLock<Option<HashMap<String, HashMap<String, ()>>>> = LazyLock::new(|| {
    let raw: serde_json::Value = serde_json::from_str(
        include_str!("../../../../data/additional_specs/Policies.json")
    ).ok()?;
    let obj = raw.as_object()?;
    let mut map = HashMap::new();
    for (service, svc_val) in obj {
        let actions = svc_val.get("Actions")?.as_object()?;
        let action_map: HashMap<String, ()> = actions.keys().map(|k| (k.to_lowercase(), ())).collect();
...
impl CfnLintRule for W3037 {
    fn id(&self) -> &str { "W3037" }
    fn short_description(&self) -> &str { "Check IAM Permission configuration" }
    fn description(&self) -> &str { "Check for valid IAM Permissions" }
...
fn prepend_resource_path(name: &str, path: &[String]) -> Vec<String> {
    let mut full = vec!["Resources".to_string(), name.to_string(), "Properties".to_string()];
...
fn has_serverless_transform(root: &AstNode) -> bool {
    match root.get("Transform") {
        ...
        Some(AstNode::Array(arr)) => arr.elements.iter().any(|e| {
            e.as_str().map(|s| s.contains("Serverless")).unwrap_or(false)
        }),
        _ => false,
    }
}
...
fn collect_actions(node: &AstNode) -> Vec<(String, Span)> {
    match node {
        ...
        AstNode::Array(arr) => arr.elements.iter().filter_map(|e| {
