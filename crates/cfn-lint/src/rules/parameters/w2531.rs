...
impl CfnLintRule for W2531 {
    fn id(&self) -> &str { "W2531" }
    fn short_description(&self) -> &str { "Check if EOL Lambda Function Runtimes are used" }
    ...
    fn validate(
        ...
        let now = Self::today();
        let deprecated = rt_data.get("deprecated").and_then(|v| v.as_str()).unwrap_or("");
        let create_block = rt_data.get("create-block").and_then(|v| v.as_str()).unwrap_or("");
        let update_block = rt_data.get("update-block").and_then(|v| v.as_str()).unwrap_or("");
        let successor = rt_data.get("successor").and_then(|v| v.as_str()).unwrap_or("unknown");
...
"#;
        ...
        let path = vec![
            ...
            "Properties".to_string(), "Runtime".to_string(),
        ];
        let validator = crate::jsonschema::Validator::new(serde_json::json!({}));
        let errors = W2531::new().validate(&validator, "Resources/AWS::Lambda::Function/Properties/Runtime", instance, &serde_json::json!({}), &path);
