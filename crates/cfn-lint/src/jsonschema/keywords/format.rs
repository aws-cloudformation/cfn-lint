...
static RE_AMI_ID: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"^ami-([0-9a-z]{8}|[0-9a-z]{17})$").unwrap());
static RE_SG_ID: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"^sg-([a-fA-F0-9]{8}|[a-fA-F0-9]{17})$").unwrap());
static RE_SUBNET_ID: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"^subnet-(([0-9A-Fa-f]{8})|([0-9A-Fa-f]{17}))$").unwrap());
static RE_VPC_ID: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"^vpc-(([0-9A-Fa-f]{8})|([0-9A-Fa-f]{17}))$").unwrap());
static RE_SG_NAME: LazyLock<Regex> = LazyLock::new(|| Regex::new(r#"^[a-zA-Z0-9 \._\-:/()#,@\[\]+=&;\{\}!\$\*]+$"#).unwrap());
static RE_IAM_ROLE_ARN: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"^arn:(aws|aws-cn|aws-iso|aws-iso-[a-z]{1}|aws-us-gov):iam::\d{12}:role/.*$").unwrap());
static RE_LOG_GROUP_NAME: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"^[\.\-_/#A-Za-z0-9]{1,512}\z").unwrap());
static RE_DATE_TIME: LazyLock<Regex> = LazyLock::new(|| Regex::new(
    r"^\d{4}-(0[1-9]|1[0-2])-\d{2}T([01]\d|2[0-3]):[0-5]\d:[0-5]\d(\.\d+)?(Z|[+-]([01]\d|2[0-3]):[0-5]\d)$"
).unwrap());
static RE_ACM_CERT_ARN: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"^arn:aws[a-zA-Z-]*:acm:[a-z0-9-]+:\d{12}:certificate/.+$").unwrap());
static RE_KMS_KEY_ARN: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"^arn:aws[a-zA-Z-]*:kms:[a-z0-9-]+:\d{12}:(key|alias)/.+$").unwrap());
static RE_SNS_TOPIC_ARN: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"^arn:aws[a-zA-Z-]*:sns:[a-z0-9-]+:\d{12}:.+$").unwrap());
static RE_LAMBDA_ARN: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"^arn:aws[a-zA-Z-]*:lambda:[a-z0-9-]+:\d{12}:function:.+(:.+)?$").unwrap());
static RE_KMS_KEY_ID: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"^([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}|mrk-[0-9a-f]{32}|alias/[a-zA-Z0-9/_-]+)$").unwrap());
static RE_KMS_ALIAS: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"^alias/[a-zA-Z0-9:/_-]+$").unwrap());
...
pub fn validate_format(
    ...
    let error_msg = match format_name {
        "AWS::EC2::Image.Id" | "AWS::EC2::Image::Id" => {
            if RE_AMI_ID.is_match(&s.value) { None } else {
        ...
        "AWS::EC2::SecurityGroup.Id" | "AWS::EC2::SecurityGroup::Id" => {
            if RE_SG_ID.is_match(&s.value) { None } else {
            ...
            match s.value.parse::<std::net::Ipv4Addr>() {
                ...
                Err(_) => {
                    ...
                    if parts.len() != 2 {
                        ...
                        match (parts[0].parse::<std::net::Ipv4Addr>(), parts[1].parse::<u8>()) {
        ...
        "date-time" => {
            ...
            if RE_DATE_TIME.is_match(&s.value) { None } else {
                Some(format!("'{}' is not a 'date-time'", s.value))
            }
        }
        "AWS::KMS::Key.Arn" | "AWS::KMS::Key::Arn" => {
            if RE_KMS_KEY_ARN.is_match(&s.value) { None } else {
                Some(format!("'{}' is not a 'AWS::KMS::Key.Arn'", s.value))
            }
        }
        "AWS::SNS::Topic.Arn" | "AWS::SNS::Topic::Arn" => {
            if RE_SNS_TOPIC_ARN.is_match(&s.value) { None } else {
                Some(format!("'{}' is not a 'AWS::SNS::Topic.Arn'", s.value))
            }
        }
        "AWS::ACM::Certificate.Arn" | "AWS::ACM::Certificate::Arn" => {
            if RE_ACM_CERT_ARN.is_match(&s.value) { None } else {
                Some(format!("'{}' is not a 'AWS::ACM::Certificate.Arn'", s.value))
            }
        }
        "AWS::Lambda::Function.Arn" | "AWS::Lambda::Function::Arn" => {
            if RE_LAMBDA_ARN.is_match(&s.value) { None } else {
                Some(format!("'{}' is not a 'AWS::Lambda::Function.Arn'", s.value))
            }
        }
        "AWS::KMS::Key.Id" | "AWS::KMS::Key::Id" => {
            if RE_KMS_KEY_ID.is_match(&s.value) { None } else {
                Some(format!("'{}' is not a 'AWS::KMS::Key.Id'", s.value))
            }
        }
        "AWS::KMS::Alias.AliasName" | "AWS::KMS::Alias::Name" | "AWS::KMS::Key.Alias" => {
            if RE_KMS_ALIAS.is_match(&s.value) { None } else {
                Some(format!("'{}' is not a 'AWS::KMS::Alias.AliasName'", s.value))
            }
        }
        "AWS::S3::Bucket.Name" | "AWS::S3::Bucket::Name" => {
            ...
            let val = &s.value;
            let valid = val.len() >= 3 && val.len() <= 63
                && val.bytes().all(|b| b.is_ascii_lowercase() || b.is_ascii_digit() || b == b'.' || b == b'-')
