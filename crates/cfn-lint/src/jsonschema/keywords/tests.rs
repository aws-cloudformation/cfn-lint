use super::*;
use super::helpers::*;
use crate::ast::{AstNode, Position, Span, StringNode, NumberNode, FunctionNode, ObjectEntry, ObjectNode, ArrayNode, BoolNode};
...
fn str_node(s: &str) -> AstNode {
    AstNode::String(StringNode { value: s.to_string(), span: pos() })
}
...
fn test_format_ami_invalid() {
    assert_eq!(run_format(&str_node("i-12345678"), "AWS::EC2::Image.Id").len(), 1);
}

#[test]
fn test_format_ami_invalid_uppercase() {
    // Uppercase letters beyond 'f' are not in [0-9a-z]
    assert_eq!(run_format(&str_node("ami-ABCDEF12"), "AWS::EC2::Image.Id").len(), 1);
}

#[test]
fn test_format_ami_too_short() {
    assert_eq!(run_format(&str_node("ami-1234567"), "AWS::EC2::Image.Id").len(), 1);
}
...
fn test_format_sg_valid_long() {
    assert!(run_format(&str_node("sg-0123456789abcdef0"), "AWS::EC2::SecurityGroup.Id").is_empty());
}

#[test]
fn test_format_sg_invalid() {
    assert_eq!(run_format(&str_node("vpc-12345678"), "AWS::EC2::SecurityGroup.Id").len(), 1);
}

#[test]
fn test_format_sg_invalid_value() {
    assert_eq!(run_format(&str_node("sg-dne"), "AWS::EC2::SecurityGroup.Id").len(), 1);
}
...
fn test_format_subnet_invalid() {
    assert_eq!(run_format(&str_node("sub-12345678"), "AWS::EC2::Subnet.Id").len(), 1);
}
...
fn test_format_vpc_invalid() {
    assert_eq!(run_format(&str_node("sg-12345678"), "AWS::EC2::VPC.Id").len(), 1);
}

// --- Security Group Name ---
#[test]
fn test_format_sg_name_valid() {
    assert!(run_format(&str_node("my-security-group"), "AWS::EC2::SecurityGroup.Name").is_empty());
}

#[test]
fn test_format_sg_name_empty() {
    assert_eq!(run_format(&str_node(""), "AWS::EC2::SecurityGroup.Name").len(), 1);
}

#[test]
fn test_format_sg_name_invalid_chars() {
    assert_eq!(run_format(&str_node("sg\ttab"), "AWS::EC2::SecurityGroup.Name").len(), 1);
}

// --- IAM Role ARN ---
#[test]
fn test_format_iam_role_arn_valid() {
    assert!(run_format(&str_node("arn:aws:iam::123456789012:role/MyRole"), "AWS::IAM::Role.Arn").is_empty());
}

#[test]
fn test_format_iam_role_arn_invalid() {
    assert_eq!(run_format(&str_node("arn:aws:s3:::my-bucket"), "AWS::IAM::Role.Arn").len(), 1);
}

// --- Log Group Name ---
#[test]
fn test_format_log_group_valid() {
    assert!(run_format(&str_node("/aws/lambda/my-function"), "AWS::Logs::LogGroup.Name").is_empty());
}
...
fn test_format_log_group_invalid_chars() {
    assert_eq!(run_format(&str_node("log group!"), "AWS::Logs::LogGroup.Name").len(), 1);
}

#[test]
fn test_format_log_group_too_long() {
    let long = "a".repeat(513);
    assert_eq!(run_format(&str_node(&long), "AWS::Logs::LogGroup.Name").len(), 1);
}
...
fn test_format_non_string_skipped() {
    let num = AstNode::Number(NumberNode { value: 42.0, span: pos() });
...
fn num_node(n: f64) -> AstNode {
    AstNode::Number(NumberNode { value: n, span: pos() })
}
...
fn arr_node(elems: Vec<AstNode>) -> AstNode {
    AstNode::Array(ArrayNode { elements: elems, span: pos() })
}
...
fn test_dependent_excluded_trigger_absent() {
    ...
    assert!(validate_dependent_excluded(&v, &node, &constraint, &serde_json::json!({}), &[]).is_empty());
}
...
fn test_dependent_excluded_multiple_conflicts() {
    let v = make_validator();
    let constraint = serde_json::json!({"A": ["B", "C"]});
    let node = obj_node(vec![("A", str_node("1")), ("B", str_node("2")), ("C", str_node("3"))]);
    let errs = validate_dependent_excluded(&v, &node, &constraint, &serde_json::json!({}), &[]);
...
fn test_max_properties_valid() {
    ...
    assert!(validate_max_properties(&v, &node, &serde_json::json!(2), &serde_json::json!({}), &[]).is_empty());
}
...
fn test_max_properties_exceeded() {
    let v = make_validator();
    let node = obj_node(vec![("A", str_node("1")), ("B", str_node("2")), ("C", str_node("3"))]);
    let errs = validate_max_properties(&v, &node, &serde_json::json!(2), &serde_json::json!({}), &[]);
    assert_eq!(errs.len(), 1);
...
fn test_min_properties_valid() {
    let v = make_validator();
    let node = obj_node(vec![("A", str_node("1")), ("B", str_node("2"))]);
    assert!(validate_min_properties(&v, &node, &serde_json::json!(2), &serde_json::json!({}), &[]).is_empty());
}
...
fn test_min_properties_too_few() {
    let v = make_validator();
    let node = obj_node(vec![("A", str_node("1"))]);
    let errs = validate_min_properties(&v, &node, &serde_json::json!(2), &serde_json::json!({}), &[]);
    assert_eq!(errs.len(), 1);
...
fn test_property_names_valid() {
    ...
    assert!(validate_property_names(&v, &node, &constraint, &serde_json::json!({}), &[]).is_empty());
}
...
fn test_multiple_of_valid() {
    let v = make_validator();
    assert!(validate_multiple_of(&v, &num_node(10.0), &serde_json::json!(5), &serde_json::json!({}), &[]).is_empty());
}

#[test]
fn test_multiple_of_invalid() {
    let v = make_validator();
    let errs = validate_multiple_of(&v, &num_node(7.0), &serde_json::json!(3), &serde_json::json!({}), &[]);
    assert_eq!(errs.len(), 1);
...
fn test_multiple_of_float() {
    let v = make_validator();
    assert!(validate_multiple_of(&v, &num_node(0.3), &serde_json::json!(0.1), &serde_json::json!({}), &[]).is_empty());
}
...
fn test_enum_case_insensitive_match() {
    ...
    assert!(validate_enum_case_insensitive(&v, &str_node("enabled"), &constraint, &serde_json::json!({}), &[]).is_empty());
}
...
fn test_enum_case_insensitive_no_match() {
    let v = make_validator();
    let constraint = serde_json::json!(["Enabled", "Disabled"]);
    let errs = validate_enum_case_insensitive(&v, &str_node("Unknown"), &constraint, &serde_json::json!({}), &[]);
    assert_eq!(errs.len(), 1);
...
fn test_enum_case_insensitive_non_string() {
    ...
    assert!(validate_enum_case_insensitive(&v, &num_node(2.0), &constraint, &serde_json::json!({}), &[]).is_empty());
}
...
"#;
        ...
        let ctx = Context::new(Arc::clone(&tmpl)).evolve(crate::context::ContextOptions {
            functions: Some(vec![
                "Fn::Equals".into(), "Fn::And".into(), "Fn::Or".into(),
                "Fn::Not".into(), "Condition".into(),
            ]),
            ..Default::default()
        });

        let equals_schema: serde_json::Value = serde_json::from_str(
        ...
        let args = cond.as_function().expect("should be Function node").args.as_ref();
        ...
        assert!(!real_errs.is_empty(), "Should have errors for disallowed ImportValue");
    }
}
...
"#;
        ...
        let findinmap = ast.get("Resources").unwrap().get("myInstance").unwrap()
            .get("Properties").unwrap().get("ImageId").unwrap();
