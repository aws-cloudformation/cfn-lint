...
def CreateEqualsRule(rule_id, resourceType, prop, value, error_message):
        ...
        if str(value).strip().lower() != str(expected_value).strip().lower():
            matches.append(
                cfnlint.rules.RuleMatch(
                    path,
                    error_message or 'Must equal check failed',
                ),
            )
    ...
    return CreateCustomRule(
        ...
        description='Created from the custom rules parameter. This rule will check '
        'if a property value is equal to the specified value.',
        rule_func=rule_func,
    )
...
def CreateNotEqualsRule(rule_id, resourceType, prop, value, error_message):
    def rule_func(value, expected_values, path):
        ...
        if str(value).strip().lower() == str(expected_values).strip().lower():
            matches.append(
                cfnlint.rules.RuleMatch(
                    path,
                    error_message or 'Must not equal check failed',
                ),
            )
    ...
    return CreateCustomRule(
        ...
        description='Created from the custom rules parameter. This rule will check '
        'if a property value is NOT equal to the specified value.',
        rule_func=rule_func,
    )
...
def CreateGreaterRule(rule_id, resourceType, prop, value, error_message):
        ...
        if checkInt(value.strip()) and checkInt(str(expected_value).strip()):
            if value.strip().lower() < str(expected_value).strip().lower():
                matches.append(
                    cfnlint.rules.RuleMatch(
                        path,
                        error_message or 'Greater than check failed',
                    ),
                )
        else:
            matches.append(
                cfnlint.rules.RuleMatch(
                    path,
                    error_message or 'Given values are not numeric',
                ),
            )
    ...
    return CreateCustomRule(
        ...
        description='Created from the custom rules parameter. This rule will check if '
        'a property value is greater than the specified value.',
        rule_func=rule_func,
    )
...
def CreateLesserRule(rule_id, resourceType, prop, value, error_message):
        ...
        if checkInt(value.strip()) and checkInt(str(expected_value).strip()):
            if value.strip().lower() > str(expected_value).strip().lower():
                matches.append(
                    cfnlint.rules.RuleMatch(
                        path,
                        error_message or 'Lesser than check failed',
                    ),
                )
        else:
            matches.append(
    ...
    return CreateCustomRule(
        ...
        description='Created from the custom rules parameter. This rule will check '
        'if a property value is lesser than the specified value.',
        rule_func=rule_func,
    )
...
def CreateInSetRule(rule_id, resourceType, prop, value, error_message):
    ...
    return CreateCustomRule(
        ...
        description='Created from the custom rules parameter. This rule will check '
        'if a property value exists inside a list of specified values.',
        rule_func=rule_func,
    )
...
def CreateNotInSetRule(rule_id, resourceType, prop, value, error_message):
    def rule_func(value, expected_values, path):
        ...
        if value in expected_values:
            matches.append(
                cfnlint.rules.RuleMatch(
                    path,
                    error_message or 'Not in set check failed',
                ),
            )
    ...
    return CreateCustomRule(
        ...
        value,
        error_message,
        shortdesc='Custom rule to check for if a value does not exist in a list of '
        'specified values',
        description='Created from the custom rules parameter. This rule will check if a '
...
def CreateInvalidRule(rule_id, operator):
    class InvalidRule(cfnlint.rules.CloudFormationLintRule):
        def __init__(self, rule_id, operator):
            ...
            self.description = (
                'Created from the custom rule parameter. This rule is '
                'the result of an invalid configuration of a custom rule.'
            )
