...
class Elb(CloudFormationLintRule):
    ...
    description = (
        'See if Elb Resource Properties are set correctly '
        'HTTPS has certificate HTTP has no certificate'
    )
    source_url = 'https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-ec2-elb-listener.html'  # noqa: E501
    tags = ['properties', 'elb']
        ...
        if self.get_loadbalancer_type(properties) == 'application':
            ...
            if isinstance(subnets, list):
                if len(subnets) < 2:
                    if scenario:
                        message = (
                            'You must specify at least two Subnets for '
                            'load balancers with type "application" {0}'
                        )
            ...
            if self.get_loadbalancer_type(properties) == 'network':
                if properties.get('SecurityGroups'):
                    if scenario.get('Scenario'):
                        ...
                        message = (
                            'Security groups are not supported for load '
                            'balancers with type "network" {0}'
                        )
                        matches.append(RuleMatch(path, message.format(scenario_text)))
                    else:
