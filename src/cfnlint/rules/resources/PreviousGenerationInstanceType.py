    ...
    def match(self, cfn):
        ...
        for resource_type, property_type in [
          ...
          ('AWS::ElastiCache::ReplicationGroup', 'CacheNodeType'),
            ...
            for resource_name, resource in cfn.get_resources([resource_type]).items():
                if re.search('([cmr][1-3]|cc2|cg1|cr1|g2|hi1|hs1|i2|t1)\.', resource.get('Properties', {}).get(property_type, '')):
        ...
        for resource_type, top_level_property_type, property_type in [
          ...
          ('AWS::Elasticsearch::Domain', 'ClusterConfig', 'InstanceType'),
            ...
            for resource_name, resource in cfn.get_resources([resource_type]).items():
                if re.search('([cmr][1-3]|cc2|cg1|cr1|g2|hi1|hs1|i2|t1)\.', resource.get('Properties', {}).get(top_level_property_type, {}).get(property_type, '')):
