    ...
    def _test_cluster_settings(
        ...
        if self.is_cluster_enabled(
            ...
            if bool_compare(automatic_failover, False):
                ...
                if scenario is None:
                    message = (
                        '"AutomaticFailoverEnabled" must be misssing or True '
                        'when setting up a cluster at {0}'
                    )
                    results.append(
                        RuleMatch(
                            pathmessage,
                    ...
                    message = (
                        '"AutomaticFailoverEnabled" must be misssing or '
                        'True when setting up a cluster when {0} at {1}'
                    )
            ...
            if not num_node_groups:
                ...
                if num_cache_nodes <= 1:
                    ...
                    if scenario is None:
                        message = (
                            '"NumCacheClusters" must be greater than one when '
                            'creating a cluster at {0}'
                        )
                        results.append(
                            RuleMatch(
                                pathmessage,
                        ...
                        message = (
                            '"NumCacheClusters" must be greater than one when '
                            'creating a cluster when {0} at {1}'
                        )
                        scenario_text = ' and '.join(
                            [
                                'when condition "%s" is %s' % (k, v)
