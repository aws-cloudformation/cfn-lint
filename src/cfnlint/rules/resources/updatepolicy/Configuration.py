...
class Configuration(CloudFormationLintRule):
    ...
    source_url = 'https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-attribute-updatepolicy.html'  # noqa: E501
    ...
    def check_value(self, value, path, **kwargs):
        ...
        if prim_type == 'List':
            ...
            if not isinstance(
                value,
                self.valid_attributes.get('primitive_types').get(prim_type),
            ):
                matches.append(
                    RuleMatch(
                        path,
                        default_message.format(kwargs.get('key_name'), prim_type),
                    ),
                )
