...
class InstanceSize(CloudFormationLintRule):
    ...
    source_url = 'https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/Concepts.DBInstanceClass.html'  # noqa: E501
    tags = ['resources', 'rds']
    ...
    def get_resources(self, cfn):
        ...
        for resource_name, resource_values in cfn.get_resources(
            ...
            # Properties items_safe heps remove conditions and focusing on
            # the actual values and scenarios
            for prop_safe, prop_path_safe in properties.items_safe(path):
                ...
                if isinstance(engine, str) and isinstance(inst_class, str):
                    ...
                    self.logger.debug(
                        'Skip evaluation based on [Engine] or [DBInstanceClass] '
                        'not being strings.',
                    )
    ...
    def check_db_config(self, properties, region):
        ...
        if db_license in self.valid_instance_types:
                ...
                if region in self.valid_instance_types[db_license][db_engine]:
                    if (
                        ...
                        valid_values = ', '.join(
                            map(
                                str,
                                self.valid_instance_types[db_license][db_engine][
                                    region
                                ],
                            ),
                        )
                        message = (
                            f'DBInstanceClass "{db_instance_class}" is not compatible with engine '
                            f'type "{db_engine}" and LicenseModel "{db_license}" in region '
                            f'"{region}". Use instance types [{valid_values}]'
                        )
                        matches.append(
                            RuleMatch(
            ...
            self.logger.debug(
                'Skip evaluation based on license [%s] not matching.',
                db_license,
            )
        return matches
