...
class UpdateReplacePolicyDeletionPolicyOnStatefulResourceTypes(CloudFormationLintRule):
    ...
    source_url = 'https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-attribute-deletionpolicy.html'  # noqa: E501
        ...
        for r_name, r_values in resources.items():
            if r_values.get('Type') in self.likely_stateful_resource_types:
                if not r_values.get('DeletionPolicy') or not r_values.get(
                    ...
                    message = (
                        'The default action when replacing/removing a resource '
                        'is to delete it. Set explicit values for UpdateReplacePolicy '
                        '/ DeletionPolicy on potentially stateful resource: %s'
                        % '/'.join(path)
                    )
