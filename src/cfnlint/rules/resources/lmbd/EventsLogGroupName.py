...
class EventsLogGroupName(CloudFormationLintRule):
    ...
    description = (
        'The current limit for a CloudWatch Log Group is they can have 2 '
        'subscription filters. We will look for duplicate LogGroupNames inside '
        'Subscription Filters and make sure they are within 2. '
        "This doesn't account for any other subscription filters getting set."
    )
    source_url = 'https://github.com/awslabs/serverless-application-model/blob/master/versions/2016-10-31.md#user-content-cloudwatchlogs'  # noqa: E501
    ...
    def check_events_subscription_duplicated(self, cfn):
        ...
        matches = []
        message = (
            'You can only have {} Subscription Filters per CloudWatch Log Group'.format(
                self.limit,
            )
        )
        ...
        for _, c in log_group_paths.items():
            if len(c) > self.limit:
                matches.append(RuleMatch(['Resources', c[2]], message.format()))
