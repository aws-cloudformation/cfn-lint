...
class UnwantedBasedOnValue(BasedOnValue):
    ...
    description = (
        'When certain properties have a certain value it results in other properties '
        'not being needed. This rule will validate those unwanted properties are '
        'not specified when those values are supplied'
    )
    tags = ['resources']
