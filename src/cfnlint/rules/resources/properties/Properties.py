...
class Properties(CloudFormationLintRule):
    ...
    source_url = 'https://github.com/aws-cloudformation/cfn-python-lint/blob/main/docs/cfn-resource-specification.md#properties'  # noqa: E501
    tags = ['resources']
                                ...
                                if d_k == 'Fn::GetAtt':
                                    ...
                                    if resource_name:
                                        ...
                                        if not (resource_type.startswith('Custom::')):
                                            message = (
                                                'Property {0} should be of type '
                                                'List for resource {1} at {2}'
                                            )
                                            matches.append(
                                                RuleMatch(
                                                    condition_path,
            ...
            for sub_key, sub_value in text[prop].items():
                if sub_key in cfnlint.helpers.CONDITION_FUNCTIONS:
                    ...
                    if sub_key != 'Fn::FindInMap':
                        ...
                        if sub_key == 'Fn::GetAtt':
                            ...
                            if resource_name:
                                ...
                                if not (
                                    ...
                                    message = (
                                        'Property is an object instead of List at %s'
                                        % ('/'.join(map(str, path)))
                                    )
                                    matches.append(RuleMatch(path, message))
                        ...
                        self.logger.debug(
                            'Too much logic to handle whats actually in the map '
                            '"%s" so skipping any more validation.',
                            sub_value,
                        )
        else:
            message = 'Property is an object instead of List at %s' % (
        ...
        for prop in text:
            ...
            if prop not in resourcespec:
                if prop in cfnlint.helpers.CONDITION_FUNCTIONS and len_of_text == 1:
                    ...
                    self.logger.debug(
                        'Ran into function "%s".  Skipping remaining checks',
                        prop,
                    )
                elif (
                    len(text) == 1
                    ...
                    for key in resourcespec.keys():
                        if SequenceMatcher(a=prop, b=key).ratio() > 0.8:
                            message = 'Invalid Property %s. Did you mean %s?' % (
                                '/'.join(map(str, proppath)),
                                key,
                            )
                            matches.append(RuleMatch(proppath, message))
                                    ...
                                    if ref in parameternames:
                                        ...
                                        if param_type:
                                            if (
                                                ...
                                                message = (
                                                    'Property {0} should be of '
                                                    'type List or Parameter should '
                                        ...
                                        for k in text[prop].keys():
                                            ...
                                            if def_intrinsic_type:
                                                if (
                                                    ...
                                                    message = (
                                                        'Property {0} is using {1} when '
                                                        'a List is needed for resource {2}'
                                                    )
                                                    matches.append(
                                                        RuleMatch(
                                                            proppath,
