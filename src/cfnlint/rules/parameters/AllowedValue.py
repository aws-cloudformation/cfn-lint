...
class AllowedValue(CloudFormationLintRule):
    ...
    description = (
        'Check if parameters have a valid value in case of an enumator. '
        "The Parameter's allowed values is based on the usages in property (Ref)"
    )
                        ...
                        if parameter_values:
                            for index, allowed_value in enumerate(parameter_values):
                                if str(allowed_value) not in allowed_value_specs:
                                    ...
                                    message = (
                                        'You must specify a valid allowed '
                                        'value for {0} ({1}). {2}'
                                    )
                                    matches.append(
                                        RuleMatch(
