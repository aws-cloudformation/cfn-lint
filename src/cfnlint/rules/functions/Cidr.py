...
class Cidr(CloudFormationLintRule):
    ...
    source_url = 'https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/intrinsic-function-reference-cidr.html'  # noqa: E501
    tags = ['functions', 'cidr']
                ...
                for index_key, _ in value.items():
                    if index_key not in self.supported_functions:
                        if index_key == 'Fn::If':
                            ...
                            message = (
                                'Cidr ipBlock should be Cidr Range, Ref, '
                                'GetAtt, Sub or Select for {0}'
                            )
                            matches.append(
                                RuleMatch(
                                    path,
                                    message.format('/'.join(map(str, value))),
                                ),
                            )
