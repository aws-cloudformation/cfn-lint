...
class FindInMapKeys(CloudFormationLintRule):
    ...
    source_url = 'https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/intrinsic-function-reference-findinmap.html'  # noqa: E501
                    ...
                    if isinstance(first_key, (str, int)):
                        ...
                        if mapping.get(first_key):
                            ...
                            if mapping.get(first_key, {}).get(second_key) is None:
                                message = (
                                    'FindInMap second key "{0}" doesn\'t exist '
                                    'in map "{1}" under "{2}" at {3}'
                                )
                                matches.append(
                                    RuleMatch(
                                        tree[:] + [2],
