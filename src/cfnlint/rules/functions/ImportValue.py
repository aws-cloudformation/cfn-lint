...
class ImportValue(CloudFormationLintRule):
    ...
    source_url = 'https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/intrinsic-function-reference-importvalue.html'  # noqa: E501
        ...
        for iv_obj in iv_objs:
            ...
            if iv_obj[0] in unsupported_locations:
                ...
                matches.append(
                    RuleMatch(
                        tree,
                        message.format(iv_obj[0], '/'.join(map(str, tree[:-1]))),
                    ),
                )
                    ...
                    for key, _ in iv_value.items():
                        if key not in supported_functions:
                            ...
                            matches.append(
                                RuleMatch(
                                    tree,
                                    message.format('/'.join(map(str, tree[:-1]))),
                                ),
                            )
                else:
