...
class Select(CloudFormationLintRule):
    ...
    source_url = 'https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/intrinsic-function-reference-select.html'  # noqa: E501
                ...
                if len(select_value_obj) == 2:
                    ...
                    if isinstance(index_obj, dict):
                        if len(index_obj) == 1:
                            for index_key, _ in index_obj.items():
                                if index_key not in [
                                    ...
                                    message = (
                                        'Select index should be an Integer or '
                                        'a function Ref or FindInMap for {0}'
                                    )
                                    matches.append(
                                        RuleMatch(
                                            tree,
                        ...
                        try:
                            ...
                            message = (
                                'Select index should be an Integer or a function '
                                'of Ref or FindInMap for {0}'
                            )
                            matches.append(
                                RuleMatch(
                                    tree,
