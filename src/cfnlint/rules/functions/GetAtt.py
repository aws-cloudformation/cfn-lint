...
class GetAtt(CloudFormationLintRule):
    ...
    description = (
        'Validates that GetAtt parameters are to valid resources and '
        'properties of those resources'
    )
    source_url = 'https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/intrinsic-function-reference-getatt.html'  # noqa: E501
                    ...
                    if len(restype) == 1:
                        for k in restype.keys():
                            if k not in valid_attribute_functions:
                                message = (
                                    'GetAtt only supports functions "{0}" '
                                    'for attributes at {1}'
                                )
                                matches.append(
                                    RuleMatch(
            ...
            if resname:
                if resname in valid_getatts:
                    ...
                    if any(resname.startswith(s) for s in modules.keys()):
                        LOGGER.debug(
                            'Will consider valid getatt %s because it references '
