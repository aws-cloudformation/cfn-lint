...
class Template(object):  # pylint: disable=R0904,too-many-lines
    ...
    def __init__(self, filename, template, regions=['us-east-1']):  # noqa: B006
    ...
    def build_graph(self):
        ...
        try:
            ...
            LOGGER.error(
                'Could not write the graph in DOT format. Please install '
                'either `pygraphviz` or `pydot` modules.',
            )
    ...
    def get_resources(self, resource_type=[]):  # noqa: B006
        """
        Get Resources
            ...
            if 'Type' in value:
                ...
                if isinstance(valtype, str):
                    if valtype.startswith(astrik_string_types):
                        LOGGER.debug(
                            'Cant build an appropriate getatt list from %s',
                            valtype,
                        )
    ...
    def search_deep_class(self, searchClass, includeGlobals=True):
        ...
        :return if searchText is "Ref", an array like ['Resources',
        'myInstance', 'Properties', 'ImageId', 'Ref', 'Ec2ImageId']
        """
        results = []
        ...
        if includeGlobals:
            pre_results = self._search_deep_keys(
                searchClass,
                self.transform_pre.get('Globals'),
                [],
            )
    ...
    def get_condition_values(self, template, path=[]):  # noqa: B006
    ...
    def get_values(self, obj, key, path=[]):  # noqa: B006
        """
    ...
    def is_resource_available(self, path, resource):
        ...
        if resource_condition:
            # resource conditions are always true.  If the same resource
            # condition exists in the path
            ...
            if not test_path_conditions.get(resource_condition):
                ...
                if True not in test_path_conditions.get(resource_condition):
                    ...
                    for scenario in scenarios:
                        # We care about when the resource condition is false
                        # but the REF would still exist
