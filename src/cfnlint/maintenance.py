                ...
                if key == 'botocore':
                    ...
                    if service not in botocore_cache:
                        botocore_cache[service] = json.loads(
                            get_url_content(
                                'https://raw.githubusercontent.com/boto/botocore/master/botocore/data/'  # noqa: E501
                                + service
                                + '/service-2.json',
                            ),
                        )
...
def update_documentation(rules):
    ...
    with open(filename, 'w', encoding='utf-8') as new_file:
        ...
        # Add the rules
        new_file.write(
            '(_This documentation is generated by running `cfn-lint --update-documentation`, '
            'do not alter this manually_)\n\n',
        )
        new_file.write(
        ...
        for rule in [
            ...
            config = '<br />'.join(
                '{0}:{1}:{2}'.format(key, values.get('type'), values.get('default'))
        ...
        for filename in fnmatch.filter(filenames, '*.json'):
            ...
            all_patches = jsonpatch.JsonPatch(
                cfnlint.helpers.load_resource(
                    'cfnlint.data.ExtendedSpecs.{}'.format(module),
                    file_path,
                ),
            )
            ...
            for all_patch in all_patches:
                try:
                    ...
                    LOGGER.debug(
                        'Patch (%s) not applied in region %s',
                        all_patch,
                        region,
                    )
                except jsonpatch.JsonPointerException:
        ...
        for propname, propdetails in properties.items():
            ...
            if t in ['object']:
                ...
                if propdetails.get('$ref'):
                    ...
                    LOGGER.info(
                        'Unable to handle %s object for %s property',
                        names[0],
                        propname,
                    )
            elif t == 'string':
    ...
    def process_schema(schema):
        ...
        for n, v in vtypes.items():
            ...
            if v:
                ...
                element = {
                    'op': 'add',
                    'path': '/%s/%s/Properties/%s/Value'
                    % (r_type, '.'.join(n.split('.')[0:-1]), n.split('.')[-1]),
                    'value': {
                        'ValueType': n,
                    },
                }
